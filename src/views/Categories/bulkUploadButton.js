...
import SampleFile from '../../assets/sampleFile/sampleFileBulkUpload.xlsx';
...
const BulkUploadCategory = (props) => {
  ...
  const onSubmit = async (data) => {
    ...
    setLoading(true);
    try {
      const response = await sentApi(urls?.foodCategory?.bulkUpload, formData);
      fetchData();
      reset();
      onClose();
      setSnackbarMessage(t('Category BulkUpload added successfully!'));
      setSnackbarOpen(true);
    } catch (error) {
      console.error(error);
      setSnackbarMessage(t('Error adding Category BulkUpload!'));
      setSnackbarOpen(true);
    } finally {
      setLoading(false);
      setSnackbarOpen(true);
